...
word_map build_grammar_map(const char** grammar_keys, const WordGrammarWrapper* grammar_values, int grammar_length)
{
    word_map wm;
    for (int x = 0; x < grammar_length; ++x)
    {
        auto& item          = grammar_values[x];
        wm[grammar_keys[x]] = {item.word,       item.range_from, item.range_to, item.decimal_from,
                               item.decimal_to, item.unique,     item.word_type};
    }
...
// clang-format off
AllAttributesParserWrapperBase* CreateFanucAllAttributesParser(
...
// clang-format on
{
    ...
    word_grammar.metric   = build_grammar_map(metric_grammar_keys, metric_grammar_values, metric_grammar_length);
    word_grammar.imperial = build_grammar_map(imperial_grammar_keys, imperial_grammar_values, imperial_grammar_length);

    std::vector<std::string> op(operations_length);
    for (int x = 0; x < operations_length; ++x)
        op[x] = operations[x];

    code_groups_map gcodes;
    int             total{};
    for (int x = 0; x < gcode_groups_length; ++x)
    {
        const auto& key = gcode_groups_keys[x];
        for (int y = 0; y < gcode_groups_values_length[x]; ++y)
            gcodes[std::move(gcode_groups_values[total + y])].insert(key);
        total += gcode_groups_values_length[x];
    }

    code_groups_map mcodes;
    total = 0;
    for (int x = 0; x < mcode_groups_length; ++x)
    {
        const auto& key = mcode_groups_keys[x];
        for (int y = 0; y < mcode_groups_values_length[x]; ++y)
            mcodes[std::move(mcode_groups_values[total + y])].insert(key);
        total += mcode_groups_values_length[x];
    }

    auto parser = std::make_unique<AllAttributesParser>(
        std::move(word_grammar), std::move(op), std::move(gcodes), std::move(mcodes),
...
{
public:
    explicit Attributes2WordValueWrapperVisitor(WordValueWrapper& wordValueWrapper)
...
void AllAttributesParserWrapper::GetLineMacroValues(int macro_line, int** macro_ids, double** macro_values, int* length)
{
    ...
    *length        = 0;
    ...
    auto                       finder = [&mv, &macro_line](auto it) {
        return std::find_if(it, std::cend(mv), [&macro_line](const auto& p) { return p.first.line == macro_line; });
    };
...
{
public:
    explicit Attributes2StringVisitor(std::string& value)
        : value(value)
    {
    }
